...
install_postgres()
{
    ...
    sudo -u postgres psql -c "ALTER USER postgres PASSWORD '$POSTGRES_PASSWORD';"  # could not change directory to "/root": Permission denied
    sudo -u postgres psql -c "CREATE DATABASE $DB_NAME;" # could not change directory to "/root": Permission denied
